# ADR-001 — shop-core와 notification의 비동기 통합 경계
...
### notification을 별도 프로젝트로 둔 이유

알림은 주문·결제 같은 핵심 거래의 부가 기능처럼 보일 수 있지만, 장애나 예외 상황에서는 대상자에게 상태를 전달하는 핵심 운영 책임이 된다. 결제 실패, 주문 취소, 배송 시작, 시스템 장애성 이벤트처럼 사용자의 후속 행동이나 운영자의 대응이 필요한 상황에서는 알림의 재시도, 실패 추적, DLQ, 발송 이력을 shop-core 거래 트랜잭션과 분리해서 운영할 필요가 있다.
