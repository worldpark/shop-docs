...
- **쿠폰/할인(주문 금액 계산·동시성) — 승격: Task 031**(`031-backend-shop-core-coupon-issue-query-apply-order-discount.md`): 쿠폰 발급(코드 claim)·조회(쿠폰함)·적용(주문 생성 시)·주문 할인 계산(서비스/REST). `coupons`/`user_coupons` V1 스키마 재사용, `Order.discountAmount`/`finalAmount` 훅에 할인 반영(결제/환불 자동 전파·이벤트 무변경). 동시성=조건부 UPDATE(used_at 1회용·used_count 총한도)+UNIQUE(1인1매), 분산락 불요. 생성 시 소비→취소(018)/만료(022) 시 복원. 화면(쿠폰함·체크아웃 선택 UI)은 후속 Task로 분리.
- 리뷰/평점, 위시리스트, 상품 검색/필터 고도화, 주문 내역/대시보드 등. 요구 확정 시 Task로 분해.
