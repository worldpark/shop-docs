...
- **비밀번호(007)**: `BCryptPasswordEncoder`(`SecurityConfig` 빈) 공용. 현재 비밀번호 확인은 `passwordEncoder.matches(raw, user.getPasswordHash())`. 신규 비밀번호 정책 검증은 007 가입의 `@PasswordMatches`/길이 규칙을 재사용·계승.
- **권한 변경 후 로그인 상태 정책(006/008 선례)**: `MemberService.changeRole`은 권한 변경 후 `refreshTokenStore.deleteRefresh(userId)`를 **`afterCommit()`** (Redis 비트랜잭셔널)에서 호출해 기존 세션을 무효화한다. **비밀번호 변경/탈퇴도 동일 선례를 따른다**(보안상 다른 기기 세션 강제 만료).
...
- **비밀번호 변경(PATCH `/api/v1/members/me/password`, View `POST /account/password`)**
  ...
  - 변경 성공 시 **`refreshTokenStore.deleteRefresh(userId)`를 `afterCommit()`** 으로 호출(타 기기 세션 강제 만료 — changeRole 선례). 현재 access 토큰 blacklist 여부는 plan 확정(권장: refresh 삭제로 재로그인 유도, access는 잔여 TTL 후 만료).
...
- **탈퇴(DELETE `/api/v1/members/me`, View `POST /account/withdraw`)**
  ...
  - 탈퇴 후 **인증 차단**: `authenticate`/`MemberUserDetailsService`(View formLogin)에서 탈퇴 사용자 로그인 거부, `JwtAuthenticationFilter` 경유 요청도 탈퇴 사용자면 거부(plan 확정 — UserDetails `enabled=false` 또는 서비스 가드). `refreshTokenStore.deleteRefresh(userId)` `afterCommit()`로 기존 세션 무효화.
...
- **로그인 상태 정책 재사용**: 비밀번호 변경/탈퇴 시 refresh 무효화는 008 `changeRole`의 `afterCommit()` 패턴 재사용(새 정책 설계 금지).
...
- (수정) `member/repository/MemberRepository.java` — 탈퇴 상태 고려 조회(예: 활성 사용자 조회, 인증 가드)
...
- (수정) `security/**` — `/account/**`·`/api/v1/members/me/**` `authenticated`, 탈퇴 사용자 인증 차단(`MemberUserDetailsService`/authenticate 가드)
- (재사용·무변경) `RedisRefreshTokenStore`/`RedisProperties`(refresh 삭제만), `BCryptPasswordEncoder`, `@PasswordMatches`, `common/exception/BusinessException`
...
- 탈퇴 시 사용자가 **소프트 삭제**(WITHDRAWN/deletedAt)되고, 이후 **로그인이 차단**되며 기존 토큰이 무효화된다. 연관 데이터(주문 등)는 물리 삭제되지 않는다.
...
## Test
- **단위(Mockito)**: 비밀번호 변경 — 현재 비번 불일치 거부, 일치 시 `encode(newPassword)`로 재인코딩 + `user.changePassword` + `afterCommit` refresh 삭제 검증; 정보 수정 — name/phone만 반영(email/role 불변); 탈퇴 — 상태 WITHDRAWN 전이 + refresh 삭제, 물리 delete 호출 없음. `User` 도메인 메서드 단언.
- **슬라이스/통합(@DataJpaTest 또는 Testcontainers)**: `V6` 적용 + `validate` 정합. 소프트 삭제 후 활성 조회/인증 가드 동작. (email 재사용 정책 채택 시 제약/인덱스 검증.)
- **Security/REST(MockMvc)**: `PATCH /api/v1/members/me/password`·`PATCH /api/v1/members/me`·`DELETE /api/v1/members/me` — 인증 사용자 본인 성공, 비인증 401; 탈퇴 후 동일 사용자 인증/`/me` 접근 차단; 타 회원 id를 받는 경로 부재(셀프 경로만) 확인.
