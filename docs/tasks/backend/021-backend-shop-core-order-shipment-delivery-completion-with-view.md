...
- **이행 오케스트레이션(order 모듈)** — 예: `OrderFulfillmentService.deliver(shipmentId)`(`@Transactional`)
  ...
  2. 상태 권위 재검증(락 보유) — **순서 확정(정정: revision-2 — 멱등 delivered를 방어 가드보다 먼저 평가)**: ① 주문이 `cancelled`/`refunded` → `409`. ② **shipment가 이미 `delivered`이면 `markDelivered` 호출 없이 현재 응답을 `200`으로 멱등 반환**(상태 불변) — **방어 가드보다 앞**(단일/마지막 배송 rollup으로 주문이 이미 `delivered`인 배송의 재-deliver도 멱등 200에 도달해야 하므로. Acceptance "같은 배송 deliver 중복 → 멱등 200"과 정합). ③ **방어 가드(020 계승)**: 정상 흐름에서 (delivered가 아닌) 배송이 `shipping`이면 주문도 `shipping`이어야 하므로 주문이 `shipping`이 아니면 `409`(이 지점 도달 시 shipment는 `delivered` 아님 — ②에서 걸러짐). ④ shipment가 `shipping`이 아님(`preparing`/역방향) → `409`. ⑤ shipment가 `shipping`이면 진행.
