# 998. shop-core 도메인 통신 전송 토글 기반 — spi 포트 원격(REST) 전환 메커니즘

> **번호 임시 변경(2026-06-15): 034 → 998.** 범위 과확장 신호(리포트 002)로 보류·후순위화. 활성 시퀀스에서 빼두기 위한 임시 번호이며, 재개 시 원번호(034)로 환원 검토.
...
> **전송 스왑 메커니즘과 공통 인프라에 한정**한다(플래그 기반 빈 선택 + REST 클라이언트 + 내부 서비스 API 보안/에러 매핑). 특정 도메인 적용은 후속(999 member 파일럿, 구 035). **데이터 소유권/DB 분리·서비스 추출은 범위 밖**(ADR-009 이후 별 이니셔티브). 신규 도메인 기능 없음.
