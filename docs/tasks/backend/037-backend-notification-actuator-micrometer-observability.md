...
- **웹 서버 도입(핵심 결정)**: Actuator HTTP 노출을 위해 **`spring-boot-starter-web`** 을 추가한다. 근거: notification엔 애플리케이션 HTTP API가 없으므로, 웹 서버는 **오직 health/metrics 노출용**이다. (대안: WebFlux — 컨슈머에 리액티브 스택 신규 도입은 과함. servlet 채택.)
- **포트(plan 037에서 확정)**: shop-core가 8080을 점유하므로 notification은 **`server.port=8090` 단일 포트**에 actuator를 노출한다. 업무 엔드포인트가 없어 전용 `management.server.port` 분리의 실익이 낮으므로 두지 않는다(운영 격리가 필요하면 후속에 관리 포트 추가). 검증 URL은 `http://localhost:8090/actuator/*`.
...
## Goal
notification이 표준 메트릭(JVM·Kafka 컨슈머 lag/처리·DataSource·메일)과 **CircuitBreaker 메트릭**을 Micrometer/Prometheus로 노출하고, `server.port=8090`에서 health/scrape를 제공한다. 풀컨텍스트 `test`는 그린을 유지한다(자동설정·바인더가 기존 CB 테스트·컨텍스트를 오염시키지 않음).

## 범위 (Scope)
- **의존 추가**: `spring-boot-starter-actuator`, `micrometer-registry-prometheus`, `spring-boot-starter-web`(actuator HTTP 노출용), `io.github.resilience4j:resilience4j-micrometer`(버전 BOM 위임).
- **`application.yml` management 설정**:
  - `server.port`: `${NOTIFICATION_PORT:8090}`. shop-core(8080) 충돌 회피. 업무 API 없음 — actuator 노출용. (전용 `management.server.port` 분리는 미사용 — plan 037 §4 확정.)
...
- TSDB/대시보드/트레이싱 — 보류.
- 메인 포트에 업무 HTTP API 신설 — 없음(웹 서버는 actuator 노출 전용).
- notification은 HTTP가 아니므로 k6 직접 가압 대상 아님(로드맵 Non-goals) — 변경 없음.

## 검증 방법
- 앱 기동 후 `GET http://localhost:8090/actuator/health` → **200 UP**.
- `GET http://localhost:8090/actuator/prometheus` → `jvm_*`, `kafka_consumer_*`, `resilience4j_circuitbreaker_*` 메트릭 노출 확인.
...
## plan에서 확정할 결정
1. (확정) 포트 = `server.port=8090` 단일, 노출 목록 = `health,info,prometheus`. 전용 관리 포트 미사용.
2. CB 바인더를 `ResilienceConfig`에 둘지 신규 `ObservabilityConfig`로 분리할지.
3. notification엔 spring-security가 없어 8090 actuator가 기본 open — 로컬은 무방, 운영 노출 정책(네트워크 격리/인가) 명시.
