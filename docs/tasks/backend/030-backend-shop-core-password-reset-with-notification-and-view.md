...
## Context
- **인증/비밀번호(006/007)**: `BCryptPasswordEncoder` 공용. 새 비밀번호 정책은 007 가입 규칙(`@PasswordMatches`/길이) 재사용. 이메일→**활성** 사용자 해석은 029가 신설한 `MemberRepository.findActiveByEmail`(citext, WITHDRAWN 제외)을 사용한다(`findByEmail`은 탈퇴 행도 반환하므로 부적합). 029 미선행 시 fallback은 Context 029 의존 항목 참조.
...
|---|---|---|---|---|
| `POST /api/v1/auth/password-reset/request` | public | `permitAll` | 없음(이메일만) | 재설정 요청 — **항상 동일 200**(존재/미존재 무관 단일 코드, enumeration 방지) |
...
- **이벤트 계약 정의(코드보다 먼저 — event-contract-rule)**
  ...
  - **토큰 페이로드 노출 주의(plan에 명시)**: 재설정 토큰은 이메일 본문 링크 전달에 필수라 `resetUrl`로 페이로드에 포함되고, Outbox(`event_publication`)에도 일시 직렬화된다. 이는 OTP성 **단기·1회용** 토큰이며, **유효성은 Redis 키로만 판정**되므로(TTL 30분·1회용) 만료/사용 즉시 잔존 평문은 **redeem 불가한 죽은 값**이 된다. 유효 구간의 잔존 토큰을 읽으려면 동일 DB의 **BCrypt 해시·전 PII에 이미 접근 가능한 전제**가 필요하고 토큰은 그 해시보다 덜 민감하므로 **한계 위험은 무시 가능**하다. 따라서 별도 정리 메커니즘(전역 `completion-mode` 변경 등)은 **도입하지 않는다**(근거·대안 비교는 revision 문서 참조). 위험은 TTL·일회성·미로그로 한정한다. (대안: 토큰 대신 별도 채널 — 사이드 프로젝트 범위 밖.)
- **shop-core — 요청(`POST /api/v1/auth/password-reset/request`, View `POST /password-reset`)**
  - 입력: `email`. `findActiveByEmail`(029, WITHDRAWN 제외)로 활성 사용자 조회. **존재 시**: 랜덤 토큰 생성(충분한 엔트로피) → `SHA-256(token)`을 Redis `shopcore:auth:reset:{hash}`→userId로 TTL과 함께 저장(원문 미저장) → `resetUrl`(예: `{baseUrl}/password-reset/confirm?token={token}`, **`baseUrl`은 설정 프로퍼티 — 하드코딩 금지**)·`expiresAt` 구성 → `PasswordResetRequestedEvent`를 **Outbox 발행**. **미존재/탈퇴(029)**: 토큰·이벤트 없이 no-op. **두 경우 모두 동일 응답**(REST 항상 200, "메일을 보냈습니다" 류 — enumeration 방지).
...
- (신규) shop-core `security/PasswordResetTokenStore.java` **포트 인터페이스** + `RedisPasswordResetTokenStore` 운영 구현(Redis `shopcore:auth:reset:`, SHA-256 저장/조회/삭제, TTL) + 테스트용 인메모리 Fake — `RefreshTokenStore`/`RedisRefreshTokenStore`/`FakeRefreshTokenStore` **3분할 패턴 계승**(단위 테스트가 Redis 없이 동작). **포트는 비소비 조회와 소비(삭제)를 분리한다 — 예: `peek(token)`(GET 검증용, 삭제 안 함) vs `consume(token)`(POST 성공 시 1회용 삭제). 정확한 메서드명은 plan 확정.**
- (수정) shop-core `common/config/RedisProperties.java` — `Auth`에 `resetPrefix`/`resetTtl`
- (신규) shop-core `resetUrl` 조립용 `baseUrl` 설정 프로퍼티(하드코딩 금지) — 신규 config record(예: `common/config/AppUrlProperties.java`) 또는 기존 프로퍼티 확장 + `application.yml` 항목. 정확 위치는 plan 확정
...
| 새 비밀번호 화면 | `GET /password-reset/confirm?token=...` → view `auth/password-reset-confirm`(token hidden, 무효/만료 토큰 안내). **GET의 토큰 유효성 조회는 비소비 peek — 존재/만료만 확인하고 토큰을 삭제하지 않는다(1회용 소비는 POST 성공 시에만)** |
...
- 토큰은 Redis에 SHA-256 해시로만 저장되고 원문/토큰이 로그·응답·View에 노출되지 않는다(`resetUrl`은 이메일 본문 전달 목적의 이벤트 페이로드·Outbox 직렬화에 한하며, 만료/사용 시 Redis 키 소멸로 redeem 불가한 죽은 값이 됨).
...
- **Security/REST(MockMvc)**: `/api/v1/auth/password-reset/request`·`/confirm` 모두 `permitAll`(비로그인). **request는 존재/미존재 이메일 모두 항상 200(단일 코드 — enumeration 방지) + 동일 본문**, 잘못된 토큰 confirm 400, 성공 후 새 비번 로그인 가능.
