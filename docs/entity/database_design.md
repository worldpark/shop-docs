...
> 스키마의 정본(source of truth)은 Flyway 마이그레이션(`V1__init_schema.sql`)이며, 이 문서는 그 구조와 설계 의도를 설명한다.
...
| 마이그레이션 | Flyway |
| 범위 | 회원 · 카탈로그(옵션/재고) · 장바구니 · 주문 · 결제(mock) · 쿠폰 · 리뷰 · 관리자 |
| 테이블 수 | 19 (도메인 18 + Outbox 인프라 1) |
| 도메인 수 | 6 |
...
| 도메인 | 영역 | 테이블 | 핵심 결정 |
|--------|------|--------|-----------|
| 회원 · 인증 | 고객 | `users`, `addresses` | 역할 기반(customer/admin) |
...
| 주문 · 결제 | 거래 | `orders`, `order_items`, `order_item_option_values`, `payments` | 주문 시점 스냅샷 |
...
관계 표기: `||--o{` = 1:N, `||--o|` = 1:1, `}o--o{` = N:M(조인 테이블 `variant_values`). `Category → Category`는 카테고리 트리 자기참조.
...
| order_items | `(order_id)`, `(variant_id)` | 항목·집계 JOIN |
